...
public class Trader extends Thread implements Tradeable {
    ...
    @Override
    public void run() {
        // Trader buys and sells from the Market
        buyStock(Market.stock1, 2);
        sellStock(Market.stock2, 1);
    }

    @Override
    public void buyStock(Stock stock, int quantity) {
        if (stock.buy(quantity)) {
            portfolio.stockBought += quantity;
            System.out.println(traderName + " bought " + quantity + " of " + stock.getName());
        } else {
            System.out.println(traderName + " failed to buy " + quantity + " of " + stock.getName() + " (not enough quantity)");
        }
    }
    ...
    public void sellStock(Stock stock, int quantity) {
        stock.sell(quantity);
        portfolio.stockSold += quantity;
        System.out.println(traderName + " sold " + quantity + " of " + stock.getName());
    }
    //    long traderId;
//    String traderName;
//
...
//    }
//
//    public Trader(long traderId, String traderName, Portfolio porfolio) {
//        this.traderId = traderId;
//        this.traderName = traderName;
